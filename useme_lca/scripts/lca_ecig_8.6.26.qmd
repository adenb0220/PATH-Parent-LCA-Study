---
title: "LCA E-Cig Parenting Practice Profiles"
date: "8.6.2026"
author: "Chen Su"
format: 
  html:
...
## Latent Class Analyses (E-Cigarette) Indicator Variables

`p_rule_ec`  = Parents Have Lenient Rules for E-Cigarettes 
                (Lenient Rule = 1, Strict Rule = 0)

`p_use_ec` = Parents Uses E-Cigarettes themselves
                (Use = 1, No Use = 0)
...
cbbPalette <- c("#40B0A6", "#FFC20A", "#994F00", "#0C7BDC")

ecig <- read_csv(here("data", "selected_df_ecig_4.csv"))
ecig$caseid <- as.character(ecig$caseid)
...
lca_k1_7  <- lapply(1:7, function(k) {
  ...
  lca_enum  <- mplusObject(
    ...
    VARIABLE = glue(
    "categorical = p_rule_ec p_use_ec p_know p_rule_b p_use_b; 
     usevar = p_rule_ec p_use_ec p_know p_rule_b p_use_b;
     classes = c({k}); "),
  ...
  PLOT = 
    "type = plot3; 
     series = p_rule_ec p_use_ec p_know p_rule_b p_use_b(*);",
  
  usevariables = colnames(ecig),
  rdata = ecig)

lca_enum_fit <- mplusModeler(lca_enum, 
                            dataout=glue(here("mplus_lca_ecig", "lca_ecig.dat")),
                            modelout=glue(here("mplus_lca_ecig", "c{k}_lca_ecig.inp")) ,
...
lca_enum_fit <- mplusModeler(lca_enum, 
                            dataout=glue(here("mplus_lca_ecig_w", "lca_ecig_w.dat")),
                            modelout=glue(here("mplus_lca_ecig_w", "c{k}_lca_ecig_w.inp")) ,
                            check=TRUE, run = TRUE, hashfilename = FALSE)
})
...
output_lca_ecig <- readModels(here("mplus_lca_ecig"), quiet = TRUE)

enum_summary <- LatexSummaryTable(output_lca_ecig,                                      
...
allFit %>% 
  ...
  gt() %>%
  tab_header(
    title = md("**Model Fit Summary Table (E-Cigarette)**")) %>% 
  cols_label(
    Title = "Classes",
    Parameters = md("Par"),
    LL = md("*LL*"),
...
for (i in 1:length(output_lca_ecig)) {
  temp <- data.frame(unclass(output_lca_ecig[[i]]$parameters$probability.scale)) %>% 
...
ggplot(pp_plots,
  ...
  geom_point() + geom_line() + facet_wrap(~ model, ncol = 2) + labs(x= "", y = "Probability", title = "E-Cigarette Conditional Probability Plot (Unweighted)") +
...
output_lca_ecig_w <- readModels(here("mplus_lca_ecig_w"), quiet = TRUE)

enum_summary_w <- LatexSummaryTable(output_lca_ecig_w,                                          
...
allFit_w %>% 
  ...
  gt() %>%
  tab_header(
    title = md("**Model Fit Summary Table (E-Cigarette, Weighted)**")) %>% 
  cols_label(
    Title = "Classes",
    Parameters = md("Par"),
    LL = md("*LL*")) %>%
...
for (i in 1:length(output_lca_ecig_w)) {
  temp <- data.frame(unclass(output_lca_ecig_w[[i]]$parameters$probability.scale)) %>% 
    mutate(model = paste0(i, "-Class Model")) 
...
ggplot(pp_plots_w,
       aes(x = param, y = est, color = LatentClass, shape = LatentClass, group = LatentClass)) + 
  geom_point() + geom_line() + facet_wrap(~ model, ncol = 2) + labs(x= "", y = "Probability", , title = "E-Cigarette Conditional Probability Plot (Weighted)") +
  theme_minimal() + theme(legend.position = "none", axis.text.x = element_text(size = 6))
...
model_c4 <- readModels(here("mplus_lca_ecig", "c4_lca_ecig.out"), quiet = TRUE)
...
plot_lca_function(
  ...
  item_num = 5,
  class_num = 4,
  item_labels = c("Lenient Rules (E-Cigarette)","Parental Use (E-Cigarette)",
                  "Lack of Parental Knowledge","Lenient Rules (Burned)",
                  "Parental Use (Burned)"),
  class_labels = c( "Consistently Strict", "Strict but Use Burned",
                    "Lenient Rules", "E-Cigarette Favorable"),
  class_legend_order = c(1,3,2,4),
  plot_title = "Parenting Practice Profiles (E-Cigarette) (K = 4)"
  )
...
ggsave(here("figures","c4_ecig_plot.JPEG"),    
       dpi=300, height=6.5, width=11, units="in")    

...
allFit %>% 
  ...
  gt() %>%
  tab_header(
    title = md("**Model Fit Summary Table (E-Cig)**")) %>% 
  cols_label(
    Title = "Classes",
    Parameters = md("Par"),
    LL = md("*LL*"),
...
ggplot(pp_plots,
  ...
  geom_point() + geom_line() + facet_wrap(~ model, ncol = 2) + labs(x= "", y = "Probability", title = "E-Cig Conditional Probability Plot (Unweighted)") +
...
allFit_w %>% 
  ...
  gt() %>%
  tab_header(
    title = md("**Model Fit Summary Table (E-Cig, Weighted)**")) %>% 
  cols_label(
    Title = "Classes",
    Parameters = md("Par"),
    LL = md("*LL*")) %>%
...
ggplot(pp_plots_w,
       aes(x = param, y = est, color = LatentClass, shape = LatentClass, group = LatentClass)) + 
  geom_point() + geom_line() + facet_wrap(~ model, ncol = 2) + labs(x= "", y = "Probability", , title = "E-Cig Conditional Probability Plot (Weighted)") +
  theme_minimal() + theme(legend.position = "none", axis.text.x = element_text(size = 6))
